...
class WorkoutGenerator:
    ...
    def _select_exercises_with_scoring(self, user: Dict, body_parts: List[str],
        ...
        if not all_exercises:
            log_warning("No exercises found in database, using emergency fallback")
            fallback_exercises = []
            fallback_count = min(5, len(EMERGENCY_FALLBACK_EXERCISES))
            for ex in EMERGENCY_FALLBACK_EXERCISES[:fallback_count]:
            ...
            all_exercises = fallback_exercises
